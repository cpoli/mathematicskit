...
:mod:`mathematicskit.linalg.systems.stability` demonstrates for the normal
equations. :func:`numpy.linalg.svd` (LAPACK's ``?gesdd``) already uses
the numerically stable route, so mathematicskit wraps it directly rather than
...
from mathematicskit.linalg.core.base import SVDResult
...
def svd_decompose(a: np.ndarray) -> SVDResult:
    ...
    which is what every :mod:`mathematicskit.linalg` caller needs. Singular
