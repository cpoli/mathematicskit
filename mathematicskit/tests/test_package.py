"""Package-level smoke tests: mathematicskit imports cleanly and every declared
...
def test_top_level_import_exposes_declared_subpackages():
    for name in mk.__all__:
        assert hasattr(mk, name), f"mathematicskit.{name} listed in __all__ but not importable"
