...
``scipy.optimize`` -- mathematicskit does not reimplement it. The value-add
...
own) into the same :class:`~mathematicskit.optimization.core.base.OptimizeResult`
shape used by the hand-rolled methods in
:mod:`mathematicskit.optimization.systems.gradient_descent` and
:mod:`mathematicskit.optimization.systems.conjugate_gradient`, so all of them
...
class NewtonMethod(UnconstrainedOptimizer):
    ...
    :mod:`~mathematicskit.optimization.systems.gradient_descent` on
    ill-conditioned problems like :func:`~mathematicskit.optimization.utils.test_functions.rosenbrock`.
