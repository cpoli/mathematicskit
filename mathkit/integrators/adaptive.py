...
:func:`~mathkit.integrators.fixed_step.leapfrog_step` /
:func:`~mathkit.integrators.fixed_step.yoshida4_step` family. Those
integrators conserve invariants over long integrations *because* they take
a fixed step; naively varying ``dt`` from an embedded error estimate breaks
...
stiff-ish systems where accuracy control matters more than exact invariant
conservation; use the fixed-step symplectic integrators for long-time
Hamiltonian-style evolution.
"""
...
from mathkit.integrators.fixed_step import RHSFunc
