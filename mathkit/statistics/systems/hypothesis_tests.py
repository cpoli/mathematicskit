...
The z-tests use :class:`mathkit.probability.Normal` for the p-value
(mathkit.probability's own reference distribution); the t-test,
chi-square tests, and ANOVA reference distributions (Student's t,
:math:`\chi^2`, F) aren't part of mathkit.probability's roster (which
...
from mathkit.probability import Normal
from mathkit.statistics.core.base import HypothesisTestResult
...
def one_sample_z_test(data: np.ndarray, mu0: float, sigma: float, alternative: str = "two-sided") -> HypothesisTestResult:
    ...
    :class:`mathkit.probability.Normal`\ (0, 1). See DeGroot & Schervish,
