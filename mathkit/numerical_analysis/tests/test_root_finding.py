...
def test_bisection_matches_scipy_brentq():
    ...
    root_mathkit = Bisection(f, 1.0, 2.0, tol=1e-12).solve().root
    root_scipy = optimize.brentq(f, 1.0, 2.0, xtol=1e-12)
    assert root_mathkit == pytest.approx(root_scipy, abs=1e-9)


def test_newton_raphson_matches_scipy_newton():
    f = lambda x: x**2 - 2.0
    fprime = lambda x: 2.0 * x
    root_mathkit = NewtonRaphson(f, fprime, x0=1.0, tol=1e-14).solve().root
    root_scipy = optimize.newton(f, x0=1.0, fprime=fprime, tol=1e-14)
    assert root_mathkit == pytest.approx(root_scipy, abs=1e-12)
