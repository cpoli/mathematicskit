"""Plotting helpers for mathkit.fractals_chaos: escape-time heatmaps,
...
def plot_escape_time(result, ax=None, cmap: str = "viridis"):
    ...
    result : EscapeTimeResult
        From :func:`~mathkit.fractals_chaos.systems.mandelbrot_julia.mandelbrot_set`
        or :func:`~mathkit.fractals_chaos.systems.mandelbrot_julia.julia_set`.
...
def plot_box_counting(result, ax=None, label=None):
    ...
    result : BoxCountingResult
        From :func:`~mathkit.fractals_chaos.systems.box_counting.box_counting_dimension`.
    ax : matplotlib.axes.Axes, optional
        Axes to draw on; a new figure is created if omitted.
    label : str, optional
...
def plot_ifs_points(points: np.ndarray, ax=None, s: float = 0.2, color: str = "forestgreen"):
    ...
    points : ndarray, shape (n, 2)
        From :meth:`~mathkit.fractals_chaos.core.base.IteratedFunctionSystem.generate`.
    ax : matplotlib.axes.Axes, optional
        Axes to draw on; a new figure is created if omitted.
    s : float
        Marker size.
...
def plot_ca_spacetime(history: np.ndarray, ax=None, cmap: str = "binary"):
    ...
    history : ndarray, shape (n_steps + 1, width)
        From :meth:`~mathkit.fractals_chaos.core.base.CellularAutomaton.run`
        on an :class:`~mathkit.fractals_chaos.systems.cellular_automata.ElementaryCA`.
    ax : matplotlib.axes.Axes, optional
        Axes to draw on; a new figure is created if omitted.
    cmap : str
