...
preconditioner-ready implementations, so mathkit does not reimplement the
Krylov-subspace machinery itself. The value mathkit adds is the
:class:`~mathkit.linalg.core.base.IterativeSolveResult` dataclass and,
...
what :mod:`mathkit.linalg.visualizers.plots`'
:func:`~mathkit.linalg.visualizers.plots.plot_residual_history` plots.
...
from mathkit.linalg.core.base import IterativeLinearSolver, IterativeSolveResult
...
class ConjugateGradient(IterativeLinearSolver):
    ...
    max_iter : int
        ...
        :data:`~mathkit.constants.DEFAULT_MAX_ITER`; ``n`` is normally
        sufficient).
