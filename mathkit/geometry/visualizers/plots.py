"""Plotting helpers for mathkit.geometry: convex hulls, triangulations,
...
def plot_convex_hull(result, ax=None):
    ...
    result : ConvexHullResult
        From :func:`~mathkit.geometry.systems.convex_hull.convex_hull` or
        :func:`~mathkit.geometry.systems.convex_hull.graham_scan`.
...
def plot_triangulation(result, ax=None):
    ...
    result : TriangulationResult
        From :func:`~mathkit.geometry.systems.triangulation.delaunay_triangulation`.
    ax : matplotlib.axes.Axes, optional
        Axes to draw on; a new figure is created if omitted.
...
def plot_curve_frame(result, ax=None, frame_stride: int = 20, scale: float = 0.3):
    ...
    result : CurveFrameResult
        From :func:`~mathkit.geometry.systems.curves.frenet_serret_frame`
        on a 2D curve.
    ax : matplotlib.axes.Axes, optional
        Axes to draw on; a new figure is created if omitted.
    frame_stride : int
